...
{	
	...
	if(empty($brand) || empty($datetime) || empty($location)) {
		...
		//updating the table
		$result = mysqli_query($con, "UPDATE lostitems SET img ='$img',category ='$category',serial ='$serial',brand ='$brand', colour = '$colour', datetime = '$datetime', location = '$location', description = '$description' where idFound ='$idFound'");
...
<?php
//getting id from url
if (isset($_GET['idFound'])){
    $idFound = $_GET['idFound'];
    
    //selecting data associated with this particular id
    $result = mysqli_query($con, "SELECT * FROM founditems WHERE idFound = $idFound");
    ...
    <form action="editlost.php" method="POST" autocomplete="">
        ...
        <input type="hidden" name="idFound" value="<?php echo $_GET['idFound'];?>"></td>
        <button type="submit" name ="findedit" value="findedit">Submit</button>
    </form>
    </div>
    </div>
